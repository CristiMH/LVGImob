...
const ApartmentCard = ({ listing, theme = 'white' }) => {

    const navigate = useNavigate();

    const handleCardClick = () => {
        navigate(`/apartamente/${listing.id}`);
    };

    return (
        <div className={`flex flex-col ${theme === 'white' ? 'bg-white' : 'bg-[#272727] border-[1px] text-white border-black'} shadow-lg rounded-[5px]`}>
            ...
            <div className="flex flex-col gap-[10px] p-[10px] cursor-pointer pt-[10px]"  onClick={handleCardClick}>
                <div className="flex flex-col gap-[5px]">
                    ...
                    <p className={`text-[18px] font-body ${theme === 'white' ? 'text-gray-700' : 'text-gray-300'}`}>{listing.rooms} camere, etaj {listing.floor}</p>
                </div>
