...
const ApartmentModal = ({ visible, onClose, onSaved, saleTypes, locations, sectors, listingToEdit, user, conditions, heatingTypes,
  constructionTypes, plannings }) => {
  ...
  useEffect(() => {
    if (isEdit) {
      setForm({
        ...
        construction_type_id: listingToEdit?.construction_type?.id || '',
        planning_type_id: listingToEdit?.planning_type?.id || '',
        heating_type_id: listingToEdit?.heating_type?.id || '',
      });
  ...
  const handleChange = (e) => {
    ...
    if (name === 'price' || name === 'surface' || name === 'rooms' || name === 'bathrooms' || name === 'total_floors' || name === 'floor') {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      payload.append("construction_type_id", form.construction_type_id);
      payload.append("planning_type_id", form.planning_type_id);
      payload.append("heating_type_id", form.heating_type_id);
      ...
      let res;
      if (isEdit) {
        res = await api.patch(`/api/v1/apartments/${listingToEdit.id}/`, payload, {
          headers: { "Content-Type": "multipart/form-data" },
        });
      } else {
        res = await api.post(`/api/v1/apartments/`, payload, {
          headers: { "Content-Type": "multipart/form-data" },
        });
      }
            ...
            loading ? (
              ...
              <>
                <div className="flex flex-col gap-[3px]">
                  ...
                  <div className="flex flex-col gap-[5px]">
                    <label className="text-[14px] font-poppins font-[500]">Nr. de etaje clădire</label>
                    <input
                      type="text"
                      inputMode="numeric"
                      pattern="\d*"
                      ...
                      onChange={handleChange}
                      placeholder='Nr. de etaje clădire'
                      className="font-body px-[24px] py-[13px] rounded-[5px] outline-none border-[1px] border-gray-300 text-[17px]"
                    />
                  ...
                  <div className="flex flex-col gap-[5px]">
                    <label className="text-[14px] font-poppins font-[500]">Tipul de construcției</label>
                    <select
                      name="construction_type_id"
                      value={form.construction_type_id}
                      onChange={handleChange}
                      placeholder='Tipul de construcției'
                      className="font-body px-[24px] py-[13px] rounded-[5px] outline-none border-[1px] border-gray-300 text-[17px]"
                    >
                      <option value="" disabled>Tipul de construcției</option>
                      {constructionTypes && constructionTypes.map((con) => (
                        <option key={con.id} value={con.id}>
                          {con.type}
                        </option>
                      ))}
                    </select>
                    <p className={`text-[14px] text-red-400 transition-opacity duration-150 font-body ease-in ${errors?.construction_type_id ? 'opacity-100' : 'opacity-0'}`}>{errors?.construction_type_id || '.'}</p>
                  </div>

                  <div className="flex flex-col gap-[5px]">
                    <label className="text-[14px] font-poppins font-[500]">Planificare</label>
                    <select
                      name="planning_type_id"
                      value={form.planning_type_id}
                      onChange={handleChange}
                      placeholder='Planificare'
                      className="font-body px-[24px] py-[13px] rounded-[5px] outline-none border-[1px] border-gray-300 text-[17px]"
                    >
                      <option value="" disabled>Planificare</option>
                      {plannings && plannings.map((plan) => (
                        <option key={plan.id} value={plan.id}>
                          {plan.type}
                        </option>
                      ))}
                    </select>
                    <p className={`text-[14px] text-red-400 transition-opacity duration-150 font-body ease-in ${errors?.planning_type_id ? 'opacity-100' : 'opacity-0'}`}>{errors?.planning_type_id || '.'}</p>
                  </div>

                  <div className="flex flex-col gap-[5px]">
                    <label className="text-[14px] font-poppins font-[500]">Tipul de încălzire</label>
                    <select
                      ...
                      className="font-body px-[24px] py-[13px] rounded-[5px] outline-none border-[1px] border-gray-300 text-[17px]"
                    >
                      <option value="" disabled>Tipul de încălzire</option>
                      {heatingTypes && heatingTypes.map((con) => (
                        <option key={con.id} value={con.id}>
                          {con.type}
                        </option>
                      ))}
                    </select>
                    <p className={`text-[14px] text-red-400 transition-opacity duration-150 font-body ease-in ${errors?.heating_type_id ? 'opacity-100' : 'opacity-0'}`}>{errors?.heating_type_id || '.'}</p>
                  </div>
