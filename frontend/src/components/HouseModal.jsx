...
const HouseModal = ({ visible, onClose, onSaved, saleTypes, locations, sectors, listingToEdit, user }) => {
  ...
  useEffect(() => {
    if (isEdit) {
      setForm({
        ...
        rooms: listingToEdit?.rooms || '',
        total_floors: listingToEdit?.total_floors || '',
        bathrooms: listingToEdit?.bathrooms || '',
        water_installation: listingToEdit?.water_installation === true
          ? 'true'
          : listingToEdit?.water_installation === false
            ? 'false'
            : '',
        gas_installation: listingToEdit?.gas_installation === true
          ? 'true'
          : listingToEdit?.gas_installation === false
            ? 'false'
            : '',
        sewerage_installation: listingToEdit?.sewerage_installation === true
          ? 'true'
          : listingToEdit?.sewerage_installation === false
            ? 'false'
            : '',
      });
  ...
  const handleChange = (e) => {
    ...
    if (name === 'price' || name === 'surface' || name === 'rooms' || name === 'bathrooms' || name === 'total_floors') {
    ...
    if (name === 'availability' || name === 'gas_installation' || name === 'sewerage_installation' || name === 'water_installation') {
      newValue = value;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      payload.append("rooms", form.rooms);
      payload.append("total_floors", form.total_floors);
      payload.append("bathrooms", form.bathrooms);
      payload.append("water_installation", form.water_installation === 'true' ? true : form.water_installation === 'false' ? false : null);
      payload.append("gas_installation", form.gas_installation === 'true' ? true : form.gas_installation === 'false' ? false : null);
      payload.append("sewerage_installation", form.sewerage_installation === 'true' ? true : form.sewerage_installation === 'false' ? false : null);
      ...
      let res;
      if (isEdit) {
        res = await api.patch(`/api/v1/houses/${listingToEdit.id}/`, payload, {
          headers: { "Content-Type": "multipart/form-data" },
        });
      } else {
        res = await api.post(`/api/v1/houses/`, payload, {
          headers: { "Content-Type": "multipart/form-data" },
        });
      }
            ...
            loading ? (
              ...
              <>
                <div className="flex flex-col gap-[3px]">
                  ...
                  <div className="flex flex-col gap-[5px]">
                    <label className="text-[14px] font-poppins font-[500]">Nr. de camere</label>
                    <input
                      type="text"
                      inputMode="numeric"
                      pattern="\d*"
                      ...
                      value={form.rooms}
                      onChange={handleChange}
                      placeholder='Nr. de camere'
                      className="font-body px-[24px] py-[13px] rounded-[5px] outline-none border-[1px] border-gray-300 text-[17px]"
                    />
                    <p className={`text-[14px] text-red-400 transition-opacity duration-150 font-body ease-in ${errors?.rooms ? 'opacity-100' : 'opacity-0'}`}>{errors.rooms || '.'}</p>
                  </div>
                  ...
                  <div className="flex flex-col gap-[5px]">
                    <label className="text-[14px] font-poppins font-[500]">Nr. de etaje</label>
                    <input
                      type="text"
                      inputMode="numeric"
                      pattern="\d*"
                      ...
                      className="font-body px-[24px] py-[13px] rounded-[5px] outline-none border-[1px] border-gray-300 text-[17px]"
                    />
                    <p className={`text-[14px] text-red-400 transition-opacity duration-150 font-body ease-in ${errors?.total_floors ? 'opacity-100' : 'opacity-0'}`}>{errors.total_floors || '.'}</p>
                  </div>

                  <div className="flex flex-col gap-[5px]">
                    <label className="text-[14px] font-poppins font-[500]">Instalație de apă</label>
                    <select
                      name="water_installation"
                      value={form.water_installation === 'true' ? 'true' : form.water_installation === 'false' ? 'false' : ''}
                      onChange={handleChange}
                      className="font-body px-[24px] py-[13px] rounded-[5px] outline-none border-[1px] border-gray-300 text-[17px]"
                    >
                      <option value="" disabled>Instalație de apă</option>
                      <option value="true">Da</option>
                      <option value="false">Nu</option>
                    </select>
                    <p className={`text-[14px] text-red-400 transition-opacity duration-150 font-body ease-in ${errors?.water_installation ? 'opacity-100' : 'opacity-0'}`}>{errors.water_installation || '.'}</p>
                  </div>

                  <div className="flex flex-col gap-[5px]">
                    <label className="text-[14px] font-poppins font-[500]">Gazificare</label>
                    <select
                      name="gas_installation"
                      value={form.gas_installation === 'true' ? 'true' : form.gas_installation === 'false' ? 'false' : ''}
                      onChange={handleChange}
                      className="font-body px-[24px] py-[13px] rounded-[5px] outline-none border-[1px] border-gray-300 text-[17px]"
                    >
                      <option value="" disabled>Gazificare</option>
                      <option value="true">Da</option>
                      <option value="false">Nu</option>
                    </select>
                    <p className={`text-[14px] text-red-400 transition-opacity duration-150 font-body ease-in ${errors?.gas_installation ? 'opacity-100' : 'opacity-0'}`}>{errors.gas_installation || '.'}</p>
                  </div>

                  <div className="flex flex-col gap-[5px]">
                    <label className="text-[14px] font-poppins font-[500]">Canalizare</label>
                    <select
                      name="sewerage_installation"
                      value={form.sewerage_installation === 'true' ? 'true' : form.sewerage_installation === 'false' ? 'false' : ''}
                      onChange={handleChange}
                      className="font-body px-[24px] py-[13px] rounded-[5px] outline-none border-[1px] border-gray-300 text-[17px]"
                    >
                      <option value="" disabled>Canalizare</option>
                      <option value="true">Da</option>
                      <option value="false">Nu</option>
                    </select>
                    <p className={`text-[14px] text-red-400 transition-opacity duration-150 font-body ease-in ${errors?.sewerage_installation ? 'opacity-100' : 'opacity-0'}`}>{errors.sewerage_installation || '.'}</p>
                  </div>
