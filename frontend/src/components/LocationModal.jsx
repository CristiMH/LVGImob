...
const LocationModal = ({ visible, onClose, onSaved, locationToEdit }) => {
    ...
    useEffect(() => {
        if (locationToEdit) {
            setLocation(locationToEdit.location)
        } else {
            setLocation('')
        }
    }, [locationToEdit]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            let res;
            if (isEdit) {
                res = await api.patch(`/api/v1/locations/${locationToEdit.id}/`, { location: location });
            } else {
                res = await api.post(`/api/v1/locations/`, { location: location });
            }

            if (res.status === 201) {
                toast.success("Locația a fost creată.")
            } else {
                toast.success("Locația a fost modificată.")
            }
                    ...
                    {loading ? (
                        ...
                        <>
                            <div className="flex flex-col gap-[5px]">
                                <p className='text-[14px] font-poppins font-[500]'>Locația</p>
                                <input type="text" name="location" id="location"
                                    className='font-body px-[24px] py-[13px] rounded-[5px] outline-none border-[1px] border-gray-300 text-[17px]'
                                    placeholder='Locația'
                                    value={location}
                                    onChange={(e) => {setLocation(e.target.value); const { location, ...rest } = errors; setErrors(rest); }} />
                                <p className={`text-[14px] text-red-400 transition-opacity duration-150 font-body ease-in ${errors?.location ? 'opacity-100' : 'opacity-0'}`}>{errors.location || '.'}</p>
                            </div>
