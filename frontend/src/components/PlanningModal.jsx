...
const PlanningModal = ({ visible, onClose, onSaved, planningToEdit }) => {
    const isEdit = Boolean(planningToEdit);
    ...
    useEffect(() => {
        if (planningToEdit) {
            setType(planningToEdit.type)
        } else {
            setType('')
        }
        
    console.log(planningToEdit)
    }, [planningToEdit]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            let res;
            if (isEdit) {
                res = await api.patch(`/api/v1/planning-types/${planningToEdit.id}/`, { type: type });
            } else {
                res = await api.post(`/api/v1/planning-types/`, { type: type });
            }
