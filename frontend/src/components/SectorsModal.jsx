...
const SectorsModal = ({ visible, onClose, onSaved, sectorToEdit }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            let res;
            if (isEdit) {
                res = await api.patch(`/api/v1/sectors/${sectorToEdit.id}/`, { sector: sector });
            } else {
                res = await api.post(`/api/v1/sectors/`, { sector: sector });
            }

            if (res.status === 201) {
                toast.success("Sectorul a fost creat.")
            } else {
                toast.success("Sectorul a fost modificat.")
            }
        ...
        <>
            ...
            <div className="fixed top-1/2 left-1/2 z-[999] w-full max-w-[400px] overflow-y-auto max-h-[100vh] transform -translate-x-1/2 -translate-y-1/2">
                <form onSubmit={handleSubmit} className="bg-white rounded-[5px] p-[30px] w-full flex flex-col gap-[3px] h-full">
                    <h2 className="text-[20px] font-poppins font-[500] mb-[10px]">{isEdit ? 'Modifică locația' : 'Adaugă locație'}</h2>
                    ...
                    {loading ? (
                        ...
                        <>
                            <div className="flex flex-col gap-[5px]">
                                <p className='text-[14px] font-poppins font-[500]'>Sectorul</p>
                                <input type="text" name="sector" id="sector"
                                    ...
                                    value={sector}
                                    onChange={(e) => {setSector(e.target.value); const { sector, ...rest } = errors; setErrors(rest); }} />
                                <p className={`text-[14px] text-red-400 transition-opacity duration-150 font-body ease-in ${errors?.sector ? 'opacity-100' : 'opacity-0'}`}>{errors.sector || '.'}</p>
                            </div>
