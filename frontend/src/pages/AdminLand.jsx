...
const AdminLand = () => {
    ...
    useEffect(() => {
        const fetchAllData = async () => {
            try {
                const [userRes, locationsRes, sectorsRes, saleTypesRes, surfaceTypesRes] = await Promise.all([
                    api.get('/api/v1/me/'),
    ...
    const deleteLand = async (listingId) => {
        setLoading(true);
        try {
            await api.delete(`/api/v1/delete-land/${listingId}/`);
            ...
            fetchLands(pageToFetch > 0 ? pageToFetch : 1, ordering)
        } catch (error) {
    ...
    const confirm = (event, land) => {
        confirmDialog({
            ...
            acceptLabel: 'Da',
            rejectLabel: 'Nu',
            accept: () => { deleteLand(land.listing.id) }
        });
    }
                            ...
                            <>
                                ...
                                <div className="w-full">
                                    <div className="flex flex-col sm:flex-row justify-between">
                                        <p className="text-[22px] font-[500] tracking-wide font-poppins mb-[10px] order-2 sm:order-1">Terenuri ({pagination.count})</p>
                                    ...
                                    }
                                    {
                                        showLandModal && <LandModal
                                            ...
                                            onSaved={() => fetchLands(pagination.current, ordering)}
                                            listingToEdit={editingLand}
                                            saleTypes={saleTypes}
                                            locations={locations}
                                            sectors={sectors}
