...
const ApartmentDetails = () => {
    ...
    useEffect(() => {
        const fetchListing = async () => {
            try {
                const res = await api.get(`api/v1/apartments/${id}/`);
                setListing(res.data);
            } catch (error) {
                navigate('/404');
            }
    ...
    useEffect(() => {
        ...
        const fetchLikeListings = async () => {
            try {
                const res = await api.get(`api/v1/apartments/?sale_type=${listing?.listing?.sale_type?.id}`);
    ...
    return (
        <>
            {listing && (
            <Helmet>
                <title>{`${listing?.rooms} camere, ${listing?.surface} m² - ${listing?.listing?.street} | LVG Imob`}</title>
                <meta
                name="description"
                content={`Apartament cu ${listing?.rooms} camere, ${listing?.surface} m², ${listing?.bathrooms} băi, situat pe strada ${listing?.listing?.street}. ${listing?.floor} din ${listing?.total_floors}. Tip construcție: ${listing?.construction_type?.type}. Consultă detalii și contactează proprietarul pe LVG Imob.`}
                />
                <link rel="canonical" href={typeof window !== 'undefined' ? window.location.href : ''} />

                <meta property="og:title" content={`${listing?.listing?.rooms} camere - ${listing?.surface} m² | ${listing?.listing?.street}`} />
                <meta property="og:description" content={`Apartament cu ${listing?.rooms} camere, ${listing?.surface} m², situat în ${listing?.listing?.sector?.sector || listing?.listing?.location?.location}.`} />
                ...
                <div className=" xl:grid xl:grid-cols-12 justify-between gap-[40px]">
                    <div className=" xl:col-span-8 flex flex-col sm:gap-[35px] gap-[25px] w-full">
                        ...
                        <div className="flex flex-col gap-[5px]">
                            ...
                            <p className='text-[18px] font-body text-gray-700 sm:text-[20px]'>{listing?.rooms} camere • etaj {listing?.floor} • {listing?.bathrooms == 1 ? `${listing?.bathrooms} baie` : `${listing?.bathrooms} băi`}</p>
                        </div>
                        ...
                        <div className="flex flex-col gap-[25px]">
                            ...
                            <div className="flex sm:gap-[200px] gap-[15px] sm:flex-row flex-col">
                                <ul className='flex flex-col gap-[15px]'>
                                    ...
                                    <li>
                                        <div className="flex flex-col">
                                            <p className='text-[16px] font-body text-gray-700'>Tip de construcție</p>
                                            <p className='text-[16px] font-body font-[500]'>{listing?.construction_type?.type}</p>
                                        </div>
                                    </li>
                                    <li>
                                        <div className="flex flex-col">
                                            <p className='text-[16px] font-body text-gray-700'>Suprafață</p>
                                            <p className='text-[16px] font-body font-[500]'>{listing?.surface} m²</p>
                                        </div>
                                    </li>
                                    <li>
                                        <div className="flex flex-col">
                                            <p className='text-[16px] font-body text-gray-700'>Băi</p>
                                            <p className='text-[16px] font-body font-[500]'>{listing?.bathrooms == 1 ? `${listing?.bathrooms} baie` : `${listing?.bathrooms} băi`}</p>
                                        </div>
                                    </li>
                                    <li>
                                        <div className="flex flex-col">
                                            <p className='text-[16px] font-body text-gray-700'>Planificare</p>
                                            <p className='text-[16px] font-body font-[500]'>{listing?.planning_type?.type}</p>
                                        </div>
                                    </li>
                                </ul>
                                ...
                                <ul className='flex flex-col gap-[15px]'>
                                    <li>
                                        <div className="flex flex-col">
                                            <p className='text-[16px] font-body text-gray-700'>Numărul de camere</p>
                                            <p className='text-[16px] font-body font-[500]'>{listing?.rooms} camere</p>
                                        </div>
                                    </li>
                                    <li>
                                        <div className="flex flex-col">
                                            <p className='text-[16px] font-body text-gray-700'>Condiție</p>
                                            <p className='text-[16px] font-body font-[500]'>{listing?.condition?.condition}</p>
                                        </div>
                                    </li>
                                    <li>
                                        <div className="flex flex-col">
                                            <p className='text-[16px] font-body text-gray-700'>Etaj</p>
                                            <p className='text-[16px] font-body font-[500]'>{listing?.floor} din {listing?.total_floors}</p>
                                        </div>
                                    </li>
                                    <li>
                                        <div className="flex flex-col">
                                            <p className='text-[16px] font-body text-gray-700'>Încălzire</p>
                                            <p className='text-[16px] font-body font-[500]'>{listing?.heating_type?.type}</p>
                                        </div>
                                    </li>
                                </ul>
                            </div>
