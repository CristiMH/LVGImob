...
const Apartments = () => {
    ...
    useEffect(() => {
        const fetchAllData = async () => {
            try {
                const [locationsRes, sectorsRes, saleTypesRes, conditionRes, consTypesRes, planRes, heatRes] = await Promise.all([
    ...
    const title = show ? `${saleTypeText} apartamente în ${sectorText !== '' ? `${sectorText}, ` : ''} ${locationText} | LVG Imob` : 'Apartamente în Moldova | LVG Imob';
    const description = show ? `Vezi cele mai noi oferte ${saleTypeText.toLowerCase()} pentru apartamente în ${sectorText !== '' ? `${sectorText}, ` : ''} ${locationText}. Consultanță imobiliară profesionistă, filtre avansate și suport complet de la LVG Imob.` : 'Descoperă apartamente disponibile în întreaga Moldovă. Consultanță imobiliară profesionistă, filtre avansate și suport complet de la LVG Imob.';
