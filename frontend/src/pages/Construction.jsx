...
const Construction = () => {
  ...
  const [editingConType, setEditingConType] = useState(null);
  ...
  const fetchConstructionTypes = async () => {
    ...
    try {
      const res = await api.get(`/api/v1/construction-types/`);
  ...
  const deleteConstructionType = async (id) => {
    setLoading(true);

    try {
      await api.delete(`api/v1/construction-types/${id}/`);
  ...
  const confirm = (event, constructionType) => {
    confirmDialog({
      ...
      acceptLabel: 'Da',
      rejectLabel: 'Nu',
      accept: () => { deleteConstructionType(constructionType.id) }
    });
  }

  const handleEdit = (type) => {
    setEditingConType(type);
              ...
              <>
                ...
                <div className="w-full">
                  <div className="flex flex-col sm:flex-row justify-between">
                    <p className="text-[22px] font-[500] tracking-wide font-poppins mb-[10px] order-2 sm:order-1">Construcție</p>
                  ...
                  {
                    showConModal && <ConstructionModal
                      visible={showConModal}
                      onClose={() => setShowConModal(false)}
                      onSaved={() => fetchConstructionTypes()}
                      constructionToEdit={editingConType}
                    />
                  }
                </div>
              </>
