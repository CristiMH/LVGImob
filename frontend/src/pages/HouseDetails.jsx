...
const HouseDetails = () => {
    ...
    useEffect(() => {
        const fetchListing = async () => {
            try {
                const res = await api.get(`api/v1/houses/${id}/`);
                setListing(res.data);
            } catch (error) {
                navigate('/404');
            }
    ...
    useEffect(() => {
        ...
        const fetchLikeListings = async () => {
            try {
                const res = await api.get(`api/v1/houses/?sale_type=${listing?.listing?.sale_type?.id}`);
    ...
    return (
        <>
            {listing && (
                <Helmet>
                    <title>{`${listing?.listing?.street} - ${listing?.surface} m², ${listing?.rooms} camere | LVG Imob`}</title>
                    <meta
                        name="description"
                        content={`Casă de vânzare în ${listing?.listing?.sector?.sector || listing?.listing?.location?.location}, ${listing?.surface} m², ${listing?.rooms} camere, ${listing?.bathrooms} băi. Preț: ${new Intl.NumberFormat('ro-RO').format(listing?.listing?.price)} €. Vezi detalii pe LVG Imob.`}
                    />
                    <link rel="canonical" href={typeof window !== 'undefined' ? window.location.href : ''} />

                    <meta property="og:title" content={`${listing?.listing?.street} | LVG Imob`} />
                    <meta
                        property="og:description"
                        content={`Casă cu ${listing?.rooms} camere în ${listing?.listing?.sector?.sector || listing?.listing?.location?.location}, preț: ${new Intl.NumberFormat('ro-RO').format(listing?.listing?.price)} €.`}
                    />
                ...
                <div className=" xl:grid xl:grid-cols-12 justify-between gap-[40px]">
                    <div className=" xl:col-span-8 flex flex-col sm:gap-[35px] gap-[25px] w-full">
                        ...
                        <div className="flex flex-col gap-[5px]">
                            ...
                            <p className='text-[18px] font-body text-gray-700 sm:text-[20px]'>{listing?.total_floors} etaje, {listing?.rooms} camere, {listing?.land_surface} ari</p>
                        </div>
                        ...
                        <div className="flex flex-col gap-[25px]">
                            ...
                            <div className="flex sm:gap-[200px] gap-[15px] sm:flex-row flex-col">
                                <ul className='flex flex-col gap-[15px]'>
                                    ...
                                    <li>
                                        <div className="flex flex-col">
                                            <p className='text-[16px] font-body text-gray-700'>Instalație de apă</p>
                                            <p className='text-[16px] font-body font-[500]'>{listing?.water_installation ? 'Are' : 'Nu are'}</p>
                                        </div>
                                    </li>
                                    <li>
                                        <div className="flex flex-col">
                                            <p className='text-[16px] font-body text-gray-700'>Canalizare</p>
                                            <p className='text-[16px] font-body font-[500]'>{listing?.sewerage_installation ? 'Are' : 'Nu are'}</p>
                                        </div>
                                    </li>
                                    <li>
                                        <div className="flex flex-col">
                                            <p className='text-[16px] font-body text-gray-700'>Suprafața totală</p>
                                ...
                                <ul className='flex flex-col gap-[15px]'>
                                    ...
                                    <li>
                                        <div className="flex flex-col">
                                            <p className='text-[16px] font-body text-gray-700'>Gazificare</p>
                                            <p className='text-[16px] font-body font-[500]'>{listing?.gas_installation ? 'Are' : 'Nu are'}</p>
                                        </div>
                                    </li>
                                    <li>
                                        <div className="flex flex-col">
                                    ...
                                    <li>
                                        <div className="flex flex-col">
                                            <p className='text-[16px] font-body text-gray-700'>Etaje</p>
                                            <p className='text-[16px] font-body font-[500]'>{listing?.total_floors} etaje</p>
                                        </div>
                                    </li>
                                </ul>
                            </div>
