...
const Planning = () => {
  ...
  const fetchPlanningTypes = async () => {
    ...
    try {
      const res = await api.get(`/api/v1/planning-types/`);
      setPlanningTypes(res.data);
  ...
  const deletePlanningType = async (id) => {
    setLoading(true);

    try {
      await api.delete(`api/v1/planning-types/${id}/`);
  ...
  const confirm = (event, type) => {
    confirmDialog({
      ...
      acceptLabel: 'Da',
      rejectLabel: 'Nu',
      accept: () => { deletePlanningType(type.id) }
    });
  }
              ...
              <>
                ...
                <div className="w-full">
                  <div className="flex flex-col sm:flex-row justify-between">
                    <p className="text-[22px] font-[500] tracking-wide font-poppins mb-[10px] order-2 sm:order-1">Planificare</p>
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full border rounded-[5px] overflow-hidden min-w-[980px]">
                      <thead className="bg-[#0f1115] text-white font-body">
                        <tr>
                          <th className="py-[6px]">Planificare</th>
                          <th></th>
                        </tr>
                      </thead>
                      <tbody>
                        {planningTypes.map((row, idx) => {
                  ...
                  {
                    showPlanModal && <PlanningModal
                      visible={showPlanModal}
                      onClose={() => setShowPlanModal(false)}
                      onSaved={() => fetchPlanningTypes()}
                      planningToEdit={editingPlan}
                    />
                  }
                </div>
              </>
