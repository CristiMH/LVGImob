...
const SpaceDetails = () => {
    ...
    useEffect(() => {
        const fetchListing = async () => {
            try {
                const res = await api.get(`api/v1/commercial-spaces/${id}/`);
                setListing(res.data);
            } catch (error) {
                navigate('/404');
            }
    ...
    useEffect(() => {
        ...
        const fetchLikeListings = async () => {
            try {
                const res = await api.get(`api/v1/commercial-spaces/?sale_type=${listing?.listing?.sale_type?.id}`);
    ...
    return (
        <>
            {listing && (
            <Helmet>
                <title>{`${listing?.surface} m² spațiu comercial - ${listing?.listing?.street} | LVG Imob`}</title>
                <meta
                name="description"
                content={`Spațiu comercial de ${listing?.surface} m² cu ${listing?.offices} birouri, etajul ${listing?.floor}, situat pe strada ${listing?.listing?.street}. Verifică detalii, contacte și preț pe LVG Imob.`}
                />
                <link rel="canonical" href={typeof window !== 'undefined' ? window.location.href : ''} />

                <meta property="og:title" content={`${listing?.surface} m² spațiu comercial - ${listing?.listing?.street}`} />
                <meta property="og:description" content={`Spațiu cu ${listing?.offices} birouri, ${listing?.surface} m², etaj ${listing?.floor}, disponibil pe LVG Imob.`} />
                ...
                <div className=" xl:grid xl:grid-cols-12 justify-between gap-[40px]">
                    <div className=" xl:col-span-8 flex flex-col sm:gap-[35px] gap-[25px] w-full">
                        ...
                        <div className="flex flex-col gap-[5px]">
                            ...
                            <p className='text-[18px] font-body text-gray-700 sm:text-[20px]'>etaj {listing?.floor}, {listing?.offices == 1 ? `${listing?.offices} birou` : `${listing?.offices} birouri`} </p>
                        </div>
                                ...
                                <ul className='flex flex-col gap-[15px]'>
                                    ...
                                    <li>
                                        <div className="flex flex-col">
                                            ...
                                            <p className='text-[16px] font-body font-[500]'>Etajul {listing?.floor}</p>
                                        </div>
                                    </li>
                                    <li>
                                        <div className="flex flex-col">
                                            <p className='text-[16px] font-body text-gray-700'>Numărul de birouri</p>
                                            <p className='text-[16px] font-body font-[500]'>{listing?.offices == 1 ? `${listing?.offices} oficiu` : `${listing?.offices} oficii`}</p>
                                        </div>
                                    </li>
                                </ul>
                            </div>
